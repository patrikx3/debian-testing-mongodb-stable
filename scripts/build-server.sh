...
echo "Works like command, use a tag: sudo ./scripts/build-server.sh r4.2.7"
echo
...
#if [ -z "${2}" ]; then
#    echo "The second argument must be the MONGODB_RELEASE for example 'r4.2.7'"
#    exit 1
#fi
#MONGODB_RELEASE="${2}"
...
# require mongo release
if [ -z "${1}" ]; then
    echo "The first argument must be the MONGODB_RELEASE for example 'r4.2.7'"
    exit 1
fi
MONGODB_RELEASE="${1}"
...
pushd $BUILD
    ...
    pushd $MONGO
        ...
        # this is only because 4.2.7 uses 1.7.2 and
