...
# some info
#echo
#echo "Works like command, use a tag: sudo ./scripts/build-server.sh r4.4.3"
#echo
...
#if [ -z "${2}" ]; then
#    echo "The second argument must be the MONGODB_RELEASE for example 'r4.4.3'"
#    exit 1
#fi
#MONGODB_RELEASE="${2}"
...
# require mongo release
if [ -z "${1}" ]; then
    echo "The first argument must be the MONGODB_RELEASE for example '4.4.3'"
    exit 1
fi
MONGODB_RELEASE="${1}"
...
#        git checkout tags/${MONGODB_RELEASE}
        ...
        # this is only because 4.4.3 uses 1.7.2 and
